...
const resolveImageUrl = (value) => {
  ...
  return `${API_BASE_URL}${prefix}${value}`;
};
...
const TicketDetails = () => {
  ...
  const buildUpdatePayload = (overrides = {}) => {
    ...
    return {
      ...
      resolutionNotes: base.resolutionNotes || '',
      ...overrides
    };
  };
  ...
  const handleStatusChange = async (e) => {
    ...
    try {
      await updateTicket(id, buildUpdatePayload({ status: newStatus }));
  ...
  const handleAssignTechnician = async (e) => {
    ...
    try {
      await updateTicket(id, buildUpdatePayload({ assignedTo: techName }));
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      await updateTicket(id, buildUpdatePayload(editForm));
  ...
  const handleSaveResolution = async () => {
    ...
    try {
      ...
      await updateTicket(id, buildUpdatePayload({ resolutionNotes: trimmedNote }));
      setTicket(prev => ({ ...prev, resolutionNotes: trimmedNote }));
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            {isAdmin && (
              <div className="flex items-center gap-2">
                <Button variant="secondary" size="sm" className="gap-2" onClick={startEdit}>
                  <Edit2 size={14} />
                  Edit
                </Button>
          ...
          {isEditing ? (
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                <h3 className="font-bold text-slate-900 text-lg">Edit Ticket</h3>
                <div className="flex items-center gap-2">
            ...
            <>
              ...
              {ticket.resolutionNotes && (
                <div className="space-y-2">
                  <h4 className="text-sm font-bold text-slate-900">Resolution Note</h4>
                  <p className="text-slate-600 leading-relaxed whitespace-pre-wrap">
                    {ticket.resolutionNotes}
                  </p>
                </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                {ticket.images.map((img, i) => (
                  <div key={i} className="aspect-video rounded-xl overflow-hidden border border-slate-100 group cursor-pointer relative shadow-sm">
                    <img src={resolveImageUrl(img)} alt="Attachment" className="w-full h-full object-cover transition-transform group-hover:scale-105" />
          ...
          <Card className="p-6 border-primary-100 bg-primary-50/20 shadow-lg shadow-primary-100/50">
            ...
            <div className="space-y-4">
              ...
              <div className="pt-2">
                ...
                >
                  {isResolutionOpen ? 'Cancel Resolution Note' : 'Add Resolution Note'}
                </Button>
              </div>
              ...
              {isResolutionOpen && (
                <div className="space-y-2">
                  ...
                  <Button
                    ...
                    disabled={isSavingResolution || !resolutionNote.trim()}
                  >
